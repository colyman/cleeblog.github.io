...
**tl;dr**: BJMM reduces ISD complexity to $2^{0.04934n}$ by allowing overlapping index sets and exploiting $1+1=0$ in $\mathbf{F}_2$. The algorithm is a three-layer merge-join tree (COLUMN MATCH) that solves the Submatrix Matching Problem more efficiently than MMT.

<!-- more -->

## 1. 背景：信息集解码（ISD）

设 $C$ 为随机二元线性 $[n, k, d]$ 码，校验矩阵 $H \in \mathbf{F}_2^{(n-k)\times n}$。给定综合征 $s = He$，错误向量 $e \in \mathbf{F}_2^n$ 权重 $\omega = \lfloor (d-1)/2 \rfloor$（半距离解码），**综合征解码问题**要求从 $s$ 恢复 $e$。该问题是 McEliece 公钥密码安全性的核心假设。
...
def merge_join(L1, L2, Q, r, p, t):
    ...
    while i < len(L1_sorted) and j < len(L2_sorted):
        ...
        if lx < ly:
            ...
            # Collision: labels match → check weight
...
def proj(v, Q, r):
    """Compute (Qv)[0:r] as a bits (integer)."""
...
**Step 1（第 3 层）**：对 $i = 1, \ldots, 4$，随机划分 $[k+\ell] = P_{i,1} \cup P_{i,2}$，构造不相交基列表
...
def column_match(Q, s, p, eps1, eps2):
    ...
    for i in range(4):
        ...
        t2 = random_bits(r2(p, ell, eps1, eps2))
        L2 = merge_join(B1, B2, Q, r2_val, p2, t2)
        L2_lists.append(L2)

    # Layer 2: merge pairs to form L(1)
    t1 = random_bits(r1(p, ell, eps1))
    L1_1 = merge_join(L2_lists[0], L2_lists[1], Q, r1_val, p1, t1)
    L1_2 = merge_join(L2_lists[2], L2_lists[3], Q, r1_val, p1, xor_bits(t1, s[:r1_val]))
...
def build_base_list(pos_idx, zero_idx, weight_half, k_ell):
    ...
    positions in zero_idx are forced to 0."""
    vectors = []
    # Generate all weight-half combinations in pos_idx
